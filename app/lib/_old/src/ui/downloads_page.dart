...
class DownloadsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: () => _showAddDialog(context, ref), icon: const Icon(Icons.add), label: const Text('Nueva descarga')),
      body: Padding(
        ...
        child: Column(
          children: [
            statsAsync.when(
              ...
              error: (e, _) => Text('Error stats: $e'),
            ),
            const SizedBox(height: 12),
            Expanded(
              child: downloadsAsync.when(
                data: (items) => _DownloadsList(items: items),
  ...
  void _showAddDialog(BuildContext context, WidgetRef ref) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Agregar descarga'),
        content: TextField(
          controller: controller,
          decoration: const InputDecoration(hintText: 'Pega la URL de Mega o 1fichier', prefixIcon: Icon(Icons.link)),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                await ref.read(downloadsRepositoryProvider).add(url);
                if (ctx.mounted) Navigator.pop(ctx);
                ref.invalidate(downloadsListProvider);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Descarga agregada')));
              } catch (e) {
...
class _RealtimeInfoRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return sse.when(
      data: (d) {
        final speed = d.speed > 0 ? '${_DownloadsList._formatBytesStatic(d.speed.toInt())}/s' : '-';
        final eta = d.timeRemaining != null ? _DownloadsList._formatEtaStatic(d.timeRemaining!) : '-';
        return Row(
          children: [
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return sse.when(
      data: (d) {
        ...
        final size = _DownloadsList._formatBytesStatic(d.fileSize);
        final done = _DownloadsList._formatBytesStatic(d.downloadedSize);
        return Row(
          children: [
            Expanded(
              child: Column(
      ...
      loading: () {
        ...
        final size = _DownloadsList._formatBytesStatic(fallbackFileSize);
        final done = _DownloadsList._formatBytesStatic(fallbackDownloaded);
        return Row(
          children: [
            Expanded(
              child: Column(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Progreso en tiempo real vía SSE cuando esté en curso
                  ...
                  children: [
                    IconButton(
                      ...
                      onPressed: d.status == 'downloading'
                          ? () async {
                              await ref.read(downloadsRepositoryProvider).pause(d.id);
                              ref.invalidate(downloadsListProvider);
                            }
                          : null,
